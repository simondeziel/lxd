...
func (d *cephfs) Create() error {
	...
	// Mount the pool.
	err = TryMount(context.TODO(), srcPath, mountPoint, "ceph", 0, strings.Join(options, ","))
	if err != nil {
		return err
	}
...
func (d *cephfs) Mount() (bool, error) {
	...
	// Mount the pool.
	err = TryMount(context.TODO(), srcPath, GetPoolMountPath(d.name), "ceph", 0, strings.Join(options, ","))
	if err != nil {
		return false, err
	}
