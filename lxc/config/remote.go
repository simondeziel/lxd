...
func (c *Config) connectRemote(remote Remote, args *lxd.ConnectionArgs) (lxd.InstanceServer, error) {
	...
	// HTTPS
	if remote.AuthType != api.AuthenticationMethodOIDC && (args.TLSClientCert == "" || args.TLSClientKey == "") {
...
func (c *Config) getConnectionArgs(name string) (*lxd.ConnectionArgs, error) {
	...
	if remote.Protocol == "simplestreams" || remote.AuthType == api.AuthenticationMethodOIDC {
		return &args, nil
	}
